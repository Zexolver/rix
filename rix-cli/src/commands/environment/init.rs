...
pub fn handle_init(ctx: &RixContext) {
        ...
        if unsafe { libc::geteuid() != 0 } {
            if let Ok(home) = std::env::var("HOME") {
                ...
                if let Ok(content) = std::fs::read_to_string(config_path) {
                    if content.contains("scope = 'system'")
                        || content.contains("scope = \"system\"")
                    {
                        println!(
                            "🔒 System scope selected. Elevating privileges to initialize /etc/rix workspace..."
                        );
                        unsafe {
    ...
    let target_config_dir =
        if unsafe { libc::geteuid() == 0 } && std::env::var("RIX_INIT_NONINTERACTIVE").is_ok() {
    ...
    if let Err(e) = rix_core::system::sync::init_local_repo(&target_config_dir) {
        ...
        let _ =
            rix_core::system::sync::auto_commit(&target_config_dir, "rix: initialized environment");
    }
}
