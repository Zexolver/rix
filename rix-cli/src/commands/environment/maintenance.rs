...
pub fn handle_rollback(ctx: &RixContext, target_state: Option<String>) {
    ...
    cmd_restore.current_dir(&ctx.config_dir)
        .args(["restore", "--source", &target, "--worktree", "--staged", "."]);
    ...
    if !restore_output.status.success() {
        spinner.finish_and_clear();
        eprintln!("❌ Failed to restore git state from {}:\n{}", target, String::from_utf8_lossy(&restore_output.stderr));
        std::process::exit(1);
    }
    ...
    commit_cmd.current_dir(&ctx.config_dir).args(["commit", "-m", &commit_msg]);
    ...
    if let Err(e) = ctx.apply_upgrade(false) {
        spinner.finish_and_clear();
        eprintln!("❌ Failed to re-evaluate and apply rolled-back environment: {:?}", e);
        std::process::exit(1);
    }

    spinner.finish_and_clear();
    println!("⏪ Environment successfully rolled back and applied [finished in {:.2}s]", start_time.elapsed().as_secs_f64());
}
