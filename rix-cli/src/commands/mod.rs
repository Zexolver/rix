...
pub fn handle(cli: Cli, ctx: RixContext) {
    match cli.command {
        ...
        Commands::Sync { remote_url } => {
            ...
            if let Err(e) =
                rix_core::system::sync::sync_to_remote(&ctx.config_dir, remote_url.as_deref())
            {
        ...
        Commands::Install {
            ...
            group,
            description,
        } => package::handle_install(&ctx, packages, group, description),
        ...
        Commands::Profile(ProfileCommands::Add {
            installable,
            description,
        }) => profile::handle_add(&ctx, installable, description),
        Commands::Profile(ProfileCommands::Remove { installable }) => {
            profile::handle_remove(&ctx, installable)
        }
    }
}
